...
import { Camera, AlertTriangle, CheckCircle, Video, Eye, EyeOff, Bell, BellOff, Sparkles, Users } from 'lucide-react';
...
const cameraFeeds: CameraFeed[] = [
  ...
  },
  {
    ...
    status: 'active',
    aiEnabled: true,
    thumbnail: 'https://images.unsplash.com/photo-1566576721346-d4a3b4eaeb55?w=400&h=300&fit=crop'
  },
  {
    id: 'cam5',
    name: 'Parking Area',
    ...
    status: 'inactive',
    aiEnabled: false,
    thumbnail: 'https://images.unsplash.com/photo-1590674899484-d5640e854abe?w=400&h=300&fit=crop'
  },
  {
    id: 'cam6',
...
const initialAlerts: Alert[] = [
  {
    id: '1',
    cameraId: 'cam2',
    cameraName: 'Production Floor',
    type: 'safety',
    severity: 'critical',
    description: 'Worker detected without safety equipment in restricted area',
  ...
  },
  {
    ...
    type: 'face_detection',
    severity: 'medium',
    description: 'Unrecognized individual detected at entrance',
  ...
  },
  {
    ...
    type: 'anomaly',
    severity: 'high',
    description: 'Unusual activity pattern detected - potential unauthorized access',
  ...
  },
  {
    ...
    type: 'equipment',
    severity: 'medium',
    description: 'Forklift left running unattended',
  ...
  },
  {
    id: '5',
    cameraId: 'cam6',
    cameraName: 'Quality Control',
    type: 'anomaly',
    severity: 'low',
    description: 'Product placement deviation from standard procedure',
...
export function AlertPage() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const randomCamera = cameraFeeds[Math.floor(Math.random() * cameraFeeds.length)];
      const types: Alert['type'][] = ['anomaly', 'intrusion', 'safety', 'equipment', 'face_detection'];
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">AI-powered monitoring and anomaly detection</p>
        </div>
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-900 text-xl">{activeCameras}/{cameraFeeds.length}</p>
            </div>
          </div>
        </div>
                    ...
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        ...
                        <p className="text-gray-600 text-sm">
                          {alert.cameraName} • {formatTimestamp(alert.timestamp)}
                        </p>
                      </div>
                      <div className="flex items-center gap-2 flex-shrink-0 ml-4">
                        <span className={`px-3 py-1 rounded border text-sm ${getSeverityColor(alert.severity)}`}>
                          {alert.severity}
                        </span>
                      </div>
                    </div>
          ...
          <div className="flex-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
              <div className="flex items-center gap-2">
                ...
                <span className="text-gray-700">Face Recognition Model v2.1</span>
              </div>
              <div className="flex items-center gap-2">
                <CheckCircle className="w-4 h-4 text-green-600" />
                <span className="text-gray-700">Anomaly Detection Model v1.8</span>
              </div>
              <div className="flex items-center gap-2">
                <CheckCircle className="w-4 h-4 text-green-600" />
                <span className="text-gray-700">Safety Equipment Detection v1.5</span>
              </div>
              <div className="flex items-center gap-2">
                <CheckCircle className="w-4 h-4 text-green-600" />
                <span className="text-gray-700">Behavior Analysis Model v2.0</span>
              </div>
            </div>
            <p className="text-gray-600 text-sm mt-3">
              Note: These AI models are placeholders. In production, connect to your Python backend API endpoints for real-time detection.
            </p>
          </div>
        </div>
      </div>
